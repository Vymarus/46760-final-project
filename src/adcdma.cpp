...
// Credit goes to manitou48 at https://github.com/manitou48/ZERO/blob/master/adcdma.ino
...
void adc_dma(void *rxdata,  size_t hwords) {
    ...
    // Read ADC and better syncronize ADC and DMAC
...
void adc_init(){
  ...
  ADCsync();
  // See page 864: ADC Control B register
  ADC->CTRLB.reg =  ADC_CTRLB_PRESCALER_DIV64 | // 
                    ADC_CTRLB_FREERUN |         // Let it run continously
                    ADC_CTRLB_RESSEL_10BIT;     // 10-bit resolution
                                                // 
  ADCsync();
  // See page 860: ADC Control A register
  // It's here ADC can be configured to run in standby mode
...
void loop() {
	...
	uint32_t t;
	t = micros();
	// while(!dmadone && micros() - t < 2*HWORDS + 13 );  // await DMA done isr
  adcbuf[0] = analogRead(A1);
	t = micros() - t;
